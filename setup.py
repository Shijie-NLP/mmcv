...
try:
    ...
    if torch.__version__ == "parrots":
        ...
        EXT_TYPE = "parrots"
    elif (hasattr(torch, "is_mlu_available") and torch.is_mlu_available()) or os.getenv("FORCE_MLU", "0") == "1":
        from torch_mlu.utils.cpp_extension import BuildExtension

        EXT_TYPE = "pytorch"
    elif (hasattr(torch, "is_musa_available") and torch.is_musa_available()) or os.getenv("FORCE_MUSA", "0") == "1":
...
def get_extensions():
    ...
    if EXT_TYPE == "parrots":
        ...
        if os.getenv("MMCV_WITH_DIOPI", "0") == "1":
            ...
            op_files = glob.glob("./mmcv/ops/csrc/pytorch/*.cpp") + glob.glob("./mmcv/ops/csrc/pytorch/cpu/*.cpp")
        ...
        elif is_rocm_pytorch or torch.cuda.is_available() or os.getenv("FORCE_CUDA", "0") == "1":
            if is_rocm_pytorch:
            ...
            mmcv_mluops_version = get_mluops_version("./mmcv/ops/csrc/pytorch/mlu/mlu_common_helper.h")
            mlu_ops_path = os.getenv("MMCV_MLU_OPS_PATH")
            if mlu_ops_path:
                exists_mluops_version = get_mluops_version(mlu_ops_path + "/bangc-ops/mlu_op.h")
                if exists_mluops_version != mmcv_mluops_version:
                    print(
                        "the version of mlu-ops provided is %s,"
                        " while %s is needed." % (exists_mluops_version, mmcv_mluops_version)
                    )
                    exit()
                try:
                    ...
                    raise FileExistsError("mlu-ops already exists, please move it out,or rename or remove it.")
            else:
                if not os.path.exists("mlu-ops"):
                    import requests

                    mluops_url = (
                        "https://github.com/Cambricon/mlu-ops/" + "archive/refs/tags/" + mmcv_mluops_version + ".zip"
                    )
                    ...
                    exists_mluops_version = get_mluops_version("./mlu-ops/bangc-ops/mlu_op.h")
                    if exists_mluops_version != mmcv_mluops_version:
                        print(
        ...
        elif (hasattr(torch.backends, "mps") and torch.backends.mps.is_available()) or os.getenv(
            "FORCE_MPS", "0"
        ) == "1":
            ...
            try:
                ...
                extra_compile_args["cxx"] += ['-D__FILENAME__="$$(notdir $$(abspath $$<))"']
                extra_compile_args["cxx"] += [
                    "-I"
                    + importlib.util.find_spec("torch_npu").submodule_search_locations[0]
            ...
            define_macros += [("MMCV_WITH_MUSA", None), ("MUSA_ARCH", str(get_musa_arch()))]
